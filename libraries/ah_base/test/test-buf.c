...
#define S_ASSERT_FIELD_OFFSET_SIZE_EQ(UNIT, TYPE1, FIELD1, TYPE2, FIELD2)                                              \
    ...
    S_ASSERT_FIELD_OFFSET_SIZE_EQ(unit, ah_buf_t, _octets, WSABUF, buf);

    ah_unit_assert(unit, sizeof(ah_buf_t) >= sizeof(WSABUF), "ah_buf_t seems to be missing fields");

#elif AH_HAS_POSIX

    S_ASSERT_FIELD_OFFSET_SIZE_EQ(unit, ah_buf_t, _octets, struct iovec, iov_base);
    S_ASSERT_FIELD_OFFSET_SIZE_EQ(unit, ah_buf_t, _size, struct iovec, iov_len);
