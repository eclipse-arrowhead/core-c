...
static void s_should_avoid_add_overflows(ah_unit_res_t* res)
{
    ...
    {
        int64_t result = 1;
        err = ah_math_add_int64(INT64_MAX / 2, INT64_MAX / 2 + 1, &result);
    ...
    {
        int64_t result = 1;
        err = ah_math_add_int64(INT64_MIN + 4, -4, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        size_t result = 2u;
        err = ah_math_add_size(SIZE_MAX / 2u, SIZE_MAX / 2u + 1u, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        uint64_t result = 3u;
        err = ah_math_add_uint64(UINT64_MAX - 1u, 1u, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
...
static void s_should_avoid_mul_overflows(ah_unit_res_t* res)
{
    ...
    {
        int64_t result = 1;
        err = ah_math_mul_int64(INT64_MAX / 2, 2, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        int64_t result = 1;
        err = ah_math_mul_int64(INT64_MIN / 2, 2, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        size_t result = 2u;
        err = ah_math_mul_size(SIZE_MAX / 2u, 2u, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        uint64_t result = 3u;
        err = ah_math_mul_uint64(UINT64_MAX / 2u, 2u, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
...
static void s_should_avoid_sub_overflows(ah_unit_res_t* res)
{
    ...
    {
        int64_t result = 1;
        err = ah_math_sub_int64(INT64_MAX, INT64_MAX, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        int64_t result = 1;
        err = ah_math_sub_int64(INT64_MIN, INT64_MIN, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
            (void) ah_unit_assert_eq_intmax(AH_UNIT_CTX, res, result, 0);
        }
    }
    {
        size_t result = 2u;
        err = ah_math_sub_size(SIZE_MAX, SIZE_MAX, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
    ...
    {
        uint64_t result = 3u;
        err = ah_math_sub_uint64(UINT64_MAX, UINT64_MAX, &result);
        if (ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ENONE)) {
            (void) ah_unit_assert_eq_uintmax(AH_UNIT_CTX, res, result, 0);
        }
    }
...
static void s_should_detect_add_overflows(ah_unit_res_t* res)
{
    ...
    {
        int64_t result;
        err = ah_math_add_int64(INT64_MAX - 8, 9, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        int64_t result;
        err = ah_math_add_int64(INT64_MIN + 4, -5, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        size_t result;
        err = ah_math_add_size(SIZE_MAX - 2u, 3u, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        uint64_t result;
        err = ah_math_add_uint64(UINT64_MAX - 6u, 7u, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
}
...
static void s_should_detect_mul_overflows(ah_unit_res_t* res)
{
    ...
    {
        int64_t result;
        err = ah_math_mul_int64(INT64_MAX / 2, 4, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        int64_t result;
        err = ah_math_mul_int64(INT64_MIN / 3, -4, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        size_t result;
        err = ah_math_mul_size(SIZE_MAX / 5u, 6u, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        uint64_t result;
        err = ah_math_mul_uint64(UINT64_MAX / 19u, 20u, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
}
...
static void s_should_detect_sub_overflows(ah_unit_res_t* res)
{
    ...
    {
        int64_t result;
        err = ah_math_sub_int64(INT64_MAX - 8, -9, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        int64_t result;
        err = ah_math_sub_int64(INT64_MIN + 4, 5, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        size_t result;
        err = ah_math_sub_size(SIZE_MAX - 2u, SIZE_MAX - 1u, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
    {
        uint64_t result;
        err = ah_math_sub_uint64(UINT64_MAX - 6u, UINT64_MAX - 5u, &result);
        (void) ah_unit_assert_eq_err(AH_UNIT_CTX, res, err, AH_ERANGE);
    }
}
