...
/// \brief Platform buffer representation.
/// \file
///
...
/// \brief The largest size, in bytes, that can be represented by an ah_buf.
#define AH_BUF_SIZE_MAX AH_I_BUF_SIZE_MAX

/// \brief A sized reference to a contiguous chunk of memory.
///
...
struct ah_buf {
    ...
    /// \brief Size, in bytes, of memory referred to by \a base.
    size_t size;
#endif
};

/// \brief Safely initializes \a buf with \a base pointer and \a size.
///
...
///   <li><b>AH_EINVAL</b>            - \a buf is \c NULL or \a base is \c NULL and \a size is positive.
...
/// \brief Creates new ah_buf from given \a base pointer and 32-bit \a size.
///
/// \param base Pointer to chunk of memory.
/// \param size Size of chunk of memory referred to by \a base.
/// \return Created buffer.
///
/// \note In contrast to ah_buf_init(), this function never fails. This is made
...
/// \brief Checks if \a buf has a \c NULL \c base or a \c size being \c 0.
///
