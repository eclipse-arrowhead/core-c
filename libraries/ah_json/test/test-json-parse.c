...
static void s_assert_json_parse_tests(ah_unit_t* unit, const char* label, struct s_json_parse_test* tests)
{
    ...
    for (struct s_json_parse_test* test = &tests[0u]; test->source != NULL; test = &test[1u], test_i += 1u) {
        ...
        if (err != AH_ENONE) {
            ah_unit_failf(unit, "%s [%zu]:\n\tparsing failed with error `%d: %s`", label, test_i, err, ah_strerror(err));
            continue;
        }
        ah_unit_pass(unit);
        ...
        for (;; expected_val = &expected_val[1u], expected_length += 1u, actual_val = &actual_val[1u]) {
            ...
            if (expected_val->type != actual_val->type) {
                ah_unit_failf(unit, "%s [%zu:%zu]:\n\tunexpected type `%" PRIuMAX "` not matching actual type `%" PRIuMAX "`",
            ...
            if (expected_val->level != actual_val->level) {
                ah_unit_failf(unit, "%s [%zu:%zu]:\n\tunexpected level `%" PRIuMAX "` not matching actual level `%" PRIuMAX "`",
                    label, test_i, expected_length, expected_val->type, actual_val->type);
                continue;
            }
            ah_unit_pass(unit);
        ...
        if (expected_length != buf.length) {
            ah_unit_failf(unit, "%s [%zu]:\n\tunexpected value length `%zu` not matching actual length `%zu`",
...
static void s_should_parse_keywords(ah_unit_t* unit)
{
    s_assert_json_parse_tests(unit, __func__,
        (struct s_json_parse_test[]) {
            [0] = { "false ", (ah_json_val_t[]) { { "false", AH_JSON_TYPE_FALSE, 0u, 5u }, { 0u } } },
            [1] = { " null ", (ah_json_val_t[]) { { "null", AH_JSON_TYPE_NULL, 0u, 4u }, { 0u } } },
            [2] = { " true ", (ah_json_val_t[]) { { "true", AH_JSON_TYPE_TRUE, 0u, 4u }, { 0u } } },
            { 0u },
        });
}
...
static void s_should_parse_numbers(ah_unit_t* unit)
{
    s_assert_json_parse_tests(unit, __func__,
        (struct s_json_parse_test[]) {
            [0] = { "1      ", (ah_json_val_t[]) { { "1", AH_JSON_TYPE_NUMBER, 0u, 1u }, { 0u } } },
            [1] = { " -41   ", (ah_json_val_t[]) { { "-41", AH_JSON_TYPE_NUMBER, 0u, 3u }, { 0u } } },
            [2] = { "  3.67 ", (ah_json_val_t[]) { { "3.67", AH_JSON_TYPE_NUMBER, 0u, 4u }, { 0u } } },
            [3] = { "-0.53  ", (ah_json_val_t[]) { { "-0.53", AH_JSON_TYPE_NUMBER, 0u, 5u }, { 0u } } },
            [4] = { " 109E2 ", (ah_json_val_t[]) { { "109E2", AH_JSON_TYPE_NUMBER, 0u, 5u }, { 0u } } },
            [5] = { "\t7E+18", (ah_json_val_t[]) { { "7E+18", AH_JSON_TYPE_NUMBER, 0u, 4u }, { 0u } } },
            [6] = { "1.0e-24", (ah_json_val_t[]) { { "1.0e-24", AH_JSON_TYPE_NUMBER, 0u, 4u }, { 0u } } },
            { 0u },
        });
}
...
static void s_should_parse_strings(ah_unit_t* unit)
{
    s_assert_json_parse_tests(unit, __func__,
        (struct s_json_parse_test[]) {
            [0] = { "\"Hello, Arrowhead!\"", (ah_json_val_t[]) { { "Hello, Arrowhead!", AH_JSON_TYPE_STRING, 0u, 17u }, { 0u } } },
            [1] = { "\"Hello, UTF-8 ÅÄÖ!\"", (ah_json_val_t[]) { { "Hello, UTF-8 ÅÄÖ!", AH_JSON_TYPE_STRING, 0u, 20u }, { 0u } } },
            [2] = { "\"Space \\\"\\u0020\\\"\"", (ah_json_val_t[]) { { "Space \\\"\\u0020\\\"", AH_JSON_TYPE_STRING, 0u, 16u }, { 0u } } },
            { 0u },
        });
}
