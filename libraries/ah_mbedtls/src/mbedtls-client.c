...
static void s_on_open(void* cln_, ah_tcp_conn_t* conn, ah_err_t err);
static void s_on_connect(void* cln_, ah_tcp_conn_t* conn, ah_err_t err);
static void s_on_read(void* cln_, ah_tcp_conn_t* conn, ah_tcp_in_t* in, ah_err_t err);
static void s_on_write(void* cln_, ah_tcp_conn_t* conn, ah_tcp_out_t* out, ah_err_t err);
static void s_on_close(void* cln_, ah_tcp_conn_t* conn, ah_err_t err);

static ah_err_t s_close_notify(ah_mbedtls_client_t* cln, uint8_t kind);
...
ah_extern ah_err_t ah_mbedtls_client_init(ah_mbedtls_client_t* cln, ah_tcp_trans_t trans, mbedtls_ssl_config* ssl_conf, ah_mbedtls_on_handshake_done_cb on_handshake_done_cb)
{
    ...
    return ah_i_mbedtls_client_init(cln, trans, ssl_conf, on_handshake_done_cb);
}
...
ah_extern ah_tcp_trans_t ah_mbedtls_client_as_trans(ah_mbedtls_client_t* cln)
{
...
ah_extern ah_tcp_conn_t* ah_mbedtls_client_get_conn(ah_mbedtls_client_t* cln)
{
    if (cln == NULL) {
        return NULL;
    }
...
ah_err_t ah_i_mbedtls_client_init(ah_mbedtls_client_t* cln, ah_tcp_trans_t trans, mbedtls_ssl_config* ssl_conf, ah_mbedtls_on_handshake_done_cb on_handshake_done_cb)
{
    ...
    *cln = (ah_mbedtls_client_t) {
    ...
    res = mbedtls_ssl_setup(&cln->_ssl, ssl_conf);
    if (res != 0) {
...
{
    if (cln_ == NULL || conn == NULL || !ah_tcp_conn_cbs_is_valid_for_connection(obs.cbs)) {
        return AH_EINVAL;
    }
    ...
    ah_assert_if_debug(cln->_trans.vtab != NULL && cln->_trans.vtab->conn_init != NULL);
...
ah_err_t ah_i_mbedtls_conn_open(void* cln_, ah_tcp_conn_t* conn, const ah_sockaddr_t* laddr)
{
    ...
    ah_assert_if_debug(cln->_trans.vtab != NULL && cln->_trans.vtab->conn_open != NULL);
...
static void s_on_write(void* cln_, ah_tcp_conn_t* conn, ah_tcp_out_t* out, ah_err_t err)
{
    ...
    if (entry->_kind == AH_S_MBEDTLS_SEND_QUEUE_ENTRY_KIND_SHUTDOWN_WR) {
        ah_assert_if_debug(cln->_trans.vtab != NULL);
        ah_assert_if_debug(cln->_trans.vtab->conn_shutdown != NULL);
        ...
        ah_assert_if_debug(cln->_trans.vtab != NULL);
        ah_assert_if_debug(cln->_trans.vtab->conn_close != NULL);
...
{
    ah_mbedtls_client_t* cln = cln_;
    if (cln == NULL || cln->_trans.vtab == NULL || cln->_trans.vtab->conn_shutdown == NULL) {
        return AH_ESTATE;
    }
    if (conn == NULL) {
        return AH_EINVAL;
    ...
    if (ah_tcp_conn_is_writable(conn) && (flags & AH_TCP_SHUTDOWN_WR) != 0) {
        err = s_close_notify(cln, AH_S_MBEDTLS_SEND_QUEUE_ENTRY_KIND_SHUTDOWN_WR);
        if (err != AH_ENONE) {
            return err;
        }
    }
...
{
    ah_mbedtls_client_t* cln = cln_;
    if (cln == NULL || cln->_trans.vtab == NULL || cln->_trans.vtab->conn_close == NULL) {
        return AH_ESTATE;
    }
    if (conn == NULL) {
        return AH_EINVAL;
    ...
    if (ah_tcp_conn_is_writable(conn)) {
        return s_close_notify(cln, AH_S_MBEDTLS_SEND_QUEUE_ENTRY_KIND_CLOSE);
    }
...
int ah_i_mbedtls_client_write_ciphertext(void* cln_, const unsigned char* buf, size_t len)
{
    ah_err_t err;
...
int ah_i_mbedtls_client_read_ciphertext(void* cln_, unsigned char* buf, size_t len)
{
    ah_err_t err;
