...
typedef struct ah_http_iheaders ah_http_iheaders_t;
typedef struct ah_http_iheader_value_iter ah_http_iheader_value_iter_t;
...
typedef struct ah_http_oheader ah_http_oheader_t;
typedef struct ah_http_oheaders ah_http_oheaders_t;
...
struct ah_http_iheaders {
    AH_I_HTTP_IHEADERS_FIELDS
};

struct ah_http_iheader_value_iter {
    AH_I_HTTP_IHEADERS_VALUES_FIELDS
};
...
struct ah_http_oheader {
    char* name;
    char* value;
};

struct ah_http_oheaders {
    ah_http_oheader_t* pairs; // Array terminated by { NULL, * } pair.
};

struct ah_http_oreq {
    ah_http_req_line_t req_line;
    ah_http_oheaders_t headers;
    void* user_data;
};
...
ah_extern const char* ah_http_iheaders_next(ah_http_iheader_value_iter_t* iter);
