...
static void s_on_alloc(ah_tcp_sock_t* sock, ah_tcp_sock_t** conn);
...
ah_extern void ah_http_server_init(ah_http_server_t* srv, ah_tcp_trans_t trans, const ah_http_server_vtab_t* vtab)
{
    ...
    srv->_listen_ctx.alloc_cb = s_on_alloc;
}
...
{
    ah_http_server_t* srv = s_upcast_to_server(sock);
    ah_http_client_t* cnt;
    if (conn != NULL) {
        cnt = s_upcast_to_client(sock);
        cnt->_remote_addr = *remote_addr;
    }
    else {
        cnt = NULL;
    }
    srv->_vtab->on_client_accept(srv, cnt, err);
}
